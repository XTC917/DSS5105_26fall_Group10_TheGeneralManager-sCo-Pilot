...
- `snapshot`: accumulated distinct order-state observations, seeded from the tracked baseline and extended after successful `orders` uploads.
...
- `append` is also accepted for genuinely additive rows, but primary-key or other constraint conflicts roll the import back. Daily complete-table files should use `replace`.
- If validation or insertion fails, the previous business data remains unchanged and the failure is recorded in `admin_meta`.
- After a new `orders` file has been inserted successfully, each row is projected from (`order_id`, `status`, `current_stage`, `last_activity_date`) to (`order_id`, `status`, `stage`, `date`) and appended to `app.snapshot`. Duplicate four-field states are ignored with `ON CONFLICT DO NOTHING`.
- Replacement of `orders` and extension of `snapshot` occur in the same transaction. A failed import rolls both operations back; replacing `production_log` or `workshops` does not change `snapshot`.
...
|   |-- db.py              PostgreSQL read-only and administrator connections
...
The PostgreSQL path has been locally exercised for health checks, schema discovery, read-only queries, data-source endpoints, CSV/XLSX/XLS import, workshop-category expansion, import status, database constraint failures, and transactional rollback. The revised four-column snapshot schema and 223-row initial seed have also been verified locally; the post-import append behaviour still requires its final application-level idempotency test.
...
- update the schema-context description and business-rule text returned by `/api/query/schema`, which still describes the superseded outgoing-`IN_PROGRESS`/`UNION` snapshot workflow;
- test that an identical orders upload adds no duplicate snapshot state while a genuinely changed state adds one row;
